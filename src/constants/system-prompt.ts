...
- For service dependencies, give each service its own short section. In it, say in simple words what that service depends on and what breaks if the dependency is gone. Bold every service name wherever it appears. Never use a code block, text block, table, arrow chain, or ASCII flow — even as a "visual summary".
